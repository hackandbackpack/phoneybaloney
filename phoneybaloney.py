...
def fetch_api_keys():
    api_keys_url = base64.b64decode(part1).decode('utf-8') + "api_keys.json"
    try:
        response = requests.get(api_keys_url)
        response.raise_for_status()  # Raises an error for bad responses
        api_keys = response.json()
        print("Successfully fetched API keys.")
        return api_keys
    except requests.RequestException as e:
        print(f"Failed to fetch API keys from {api_keys_url}: {e}")
        return None  # Return None if there was a problem
...
# auth_to_openai(OPENAI_API_KEY)
    ...
    print("Failed to fetch API keys, exiting...")
    sys.exit(1)
    ...
    with sr.Microphone() as source:
        ...
        try:
            ...
            if retries > 0:
                ...
                speak_and_print("Are you there?", "System", "default voice name", "default language code", google_cloud_api_key)  # Use default or fetched values for voice_name and language_code
                return get_audio_input(prompt, retries-1, google_cloud_api_key)
            else:
                print("\nI am disconnecting the call now, please try us again later.")
                speak_and_print("I am disconnecting the call now, please try us again later.", "System", "default voice name", "default language code", google_cloud_api_key)  # Use default or fetched values for voice_name and language_code
                sys.exit()
...
def generate_initial_response_for_scenario():
    ...
    speak_and_print(initial_response, "System", selected_scenario['voice_name'], selected_scenario['language_code'], GOOGLE_CLOUD_API_KEY)
